...
public class HttpEngine implements Engine<HttpChannel> {
	...
	@Override
	public void onWrite(List<HttpChannel> channels, long timestamp) throws IOException {
		if (channels.size() == 1) {
	...
	@Override
	public List<Record> onRead(HttpChannel channel, long startTime, long endTime) throws IOException {
		if (!channel.hasFeed()) {
