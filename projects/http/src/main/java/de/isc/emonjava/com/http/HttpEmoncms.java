...
import de.isc.emonjava.Emoncms;
import de.isc.emonjava.Feed;
import de.isc.emonjava.Input;
import de.isc.emonjava.com.EmoncmsException;
import de.isc.emonjava.com.EmoncmsUnavailableException;
import de.isc.emonjava.com.http.HttpFeed.HttpFeedCallbacks;
import de.isc.emonjava.com.http.HttpInput.HttpInputCallbacks;
import de.isc.emonjava.com.http.json.Const;
import de.isc.emonjava.com.http.json.JsonData;
...
import de.isc.emonjava.data.DataList;
import de.isc.emonjava.data.Datatype;
import de.isc.emonjava.data.Engine;
import de.isc.emonjava.data.FeedData;
import de.isc.emonjava.data.InputData;
import de.isc.emonjava.data.Namevalue;
import de.isc.emonjava.data.Options;
...
public class HttpEmoncms implements Emoncms, HttpInputCallbacks, HttpFeedCallbacks {
	...
	@Override
	public List<InputData> loadInputList() throws EmoncmsException {
		...
		try {
			...
			List<InputData> inputList = new ArrayList<InputData>(jsonInputList.size());
			for (JsonInput jsonInput : jsonInputList) {
				HttpInput i = new HttpInput(this,
						jsonInput.getId(), jsonInput.getNodeid(), jsonInput.getName());
				...
				InputData input = new InputData(i, jsonInput.getDescription(), processList, timevalue);
	...
	@Override
	public InputData loadInput(int id) throws EmoncmsException {

		logger.debug("Requesting to load input with id: ", id);
		
		List<InputData> inputList = loadInputList();
	...
	@Override
	public List<FeedData> loadFeedList() throws EmoncmsException {
		
		logger.debug("Requesting to load feed list");

		List<JsonFeed> jsonFeedList = getJsonFeedList();

		List<FeedData> feedList = new ArrayList<FeedData>(jsonFeedList.size());
		for (JsonFeed jsonFeed : jsonFeedList) {
			HttpFeed f = new HttpFeed(this, jsonFeed.getId());

			Timevalue timevalue = null;
			if (jsonFeed.getTime() != null && jsonFeed.getValue() != null) {
				timevalue = new Timevalue(jsonFeed.getTime(), jsonFeed.getValue());
			}
			FeedData feed = new FeedData(f, 
	...
	@Override
	public FeedData loadFeed(int id) throws EmoncmsException {
