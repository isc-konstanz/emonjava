...
public class SqlFeed implements Feed {
    ...
    private static int DEFAULT_TYPE_LENGTH = 10;
    private static String[] DEFAULT_TYPES = new String[] {
            "FLOAT",
            "REAL",
            "BIGINT",
            "INT",
    ...
    private static String QUERY_CREATE = "CREATE TABLE IF NOT EXISTS %s (time INT UNSIGNED NOT NULL, data %s, code TINYINT, PRIMARY KEY (time)) ENGINE=MYISAM";
    private static String QUERY_SELECT = "SELECT * FROM %s WHERE time >= %s AND time <= %s";
    private static String QUERY_INSERT = "INSERT INTO %s (time, data, flag) VALUES(%s,%s,%s)";
    private static String QUERY_INSERT_FLAG = "INSERT INTO %s (time, flag) VALUES(%s,%s)";
    ...
    public static Feed connect(SqlCallbacks callbacks, Integer id, String table, String type) throws EmoncmsException {
        if (callbacks == null) {
        ...
        return new SqlFeed(callbacks, id, table, type);
    }

    protected SqlFeed(SqlCallbacks callbacks, Integer id, String table, String type) {
        this.callbacks = callbacks;
        this.table = table;
        this.id = id;
        
//        callbacks.doExecute(query);
    }
...
//        callbacks.doExecute(INSERT.replaceFirst("(?)", name)
//                .replaceFirst("(?)", String.valueOf(timevalue.getTime()))
//                .replaceFirst("(?)", String.valueOf(timevalue.getValue())));
    }

    @Override
    public void updateData(Timevalue timevalue)
            throws UnsupportedOperationException, EmoncmsException {
...
//
//    public SqlFeed(String dbAddress, String userName, String password, LogChannel logChannel) {
...
//    }
//    
//    public SqlFeed(String dbAddress, String userName, String password) {
//        this.dbAddress = dbAddress;
//        this.userName = userName;
//        this.password = password;
//    }
...
//        new SqlFeed("jdbc:mysql://127.0.0.1:3306/emoncms", "emoncms", "emoncms", null);
//    }
